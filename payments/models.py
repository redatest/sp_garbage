...
class Customer(StripeObject):
    ...
    def sync_current_subscription(self, cu=None):
        ...
        if sub is None:
            ...
            try:
                # update related employer download counter
                sub_obj = self.current_subscription
                employer = self.user.profile_emp
                if sub_obj.plan == "mensuel":
                    employer.down_counter = 220
                    employer.save()
                elif sub_obj.plan == "annuel":
                    employer.down_counter = 60
                    employer.save()
                else:
                    employer.down_counter = 0
                    employer.save()
